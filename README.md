...
- **Không tìm thấy Python/Node**: kiểm tra đã cài và thêm vào `PATH`
- **Thiếu API key**: kiểm tra `backend/.env`
- **Lỗi cài package**: thử chạy lại `install_all.bat` với quyền user bình thường, mạng ổn định
- **Lỗi port đang dùng**: đóng app đang chiếm port hoặc đổi port khi chạy

---
Nếu cần, mình có thể viết thêm file mẫu `backend/.env.example` chuẩn để team clone về dùng đồng nhất.
...
# Load environment variables from backend/.env (if present), independent of CWD.
...
def _get_required_env(name: str) -> str:
    ...
    raise RuntimeError(
        ...
        f"Create backend/.env and set {name}=... (see backend/.env.example)."
    )
...
def get_pinecone_index(api_key: Optional[str] = None, index_name: Optional[str] = None):
    ...
    Priority:
    - per-request params (api_key/index_name)
    - environment variables

    Index objects are cached by (api_key, index_name).
    """
    ...
    # Backward-compatible singleton cache when using env-based config
    if (
        (not api_key) and (not index_name)
        and _pinecone_index is not None
    ):
    ...
    try:
        ...
        for match in results.get('matches', []):
            ...
            if metadata:  # Only add if metadata exists
                # Validate that metadata has essential fields
                # Don't require specific keys, just pass through what exists
        ...
        safe_print(f"Error in retrieve_context: {e}")
        _append_error_log(
